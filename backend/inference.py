...
# Import model loader for real predictions
from backend.model_loader import get_model, get_device
...
def predict_image_with_face_check(image: Image.Image, mode: str) -> Tuple[str, str, float]:
    ...
    Raises:
        NoFaceDetectedError when no face is found.
    """
    cropped_face, detection = detect_and_crop_face(image)
    if detection is None:
        raise NoFaceDetectedError()
