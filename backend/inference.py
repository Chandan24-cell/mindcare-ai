...
# Set up reproducibility for research credibility
...
def _validate_cropped_face(cropped_face: Image.Image) -> bool:
    ...
    try:
        ...
        gray_image = cropped_face.convert('L')
...
def _preprocess_face_for_vit(cropped_face: Image.Image) -> torch.Tensor:
    resized_face = None
    face_array = None
    try:
        resized_face = cropped_face.convert("RGB").resize(
            VIT_INPUT_SIZE,
            Image.Resampling.BILINEAR,
        )
        face_array = np.asarray(resized_face, dtype=np.float32)
        face_array *= 1.0 / 127.5
        face_array -= 1.0
        pixel_values = (
            torch.from_numpy(face_array)
            .permute(2, 0, 1)
            .unsqueeze(0)
            .contiguous()
        )
...
def _run_vit_on_face(cropped_face: Image.Image) -> Tuple[str, str, float]:
    ...
    if not _INFERENCE_LOCK.acquire(blocking=False):
        raise MemoryBudgetExceededError(
            "Another image inference request is already running. Please retry in a moment."
        )
    ...
    try:
        ensure_memory_within_limit("ViT inference", hard_fraction=0.97)
        log_memory_usage("before ViT model access")
        model = load_vit_model()
        model.eval()
        log_memory_usage("before ViT inference")
        pixel_values = _preprocess_face_for_vit(cropped_face)
        ...
        with torch.inference_mode():
            ...
            predicted_class = int(logits.argmax(-1).item())
            confidence = float(probabilities[0][predicted_class].item())
    finally:
        del pixel_values, outputs, logits, probabilities, model
        release_unused_memory()
        log_memory_usage("after ViT inference cleanup")
...
def predict_from_sensor_data(
    ...
    5. Activity Level: Physical activity affects stress interpretation
    ...
    Args:
        ...
        activity_level: User's recent physical activity level ("low", "moderate", "high")
    ...
    # -----------------------------------------------------------------------------
    # Factor 5: Activity Level Analysis
    # -----------------------------------------------------------------------------
    # High activity can elevate HR normally, low activity may indicate fatigue
    if activity_level == "high":
        activity_stress = 1.5  # Slightly elevated due to exercise
    elif activity_level == "low":
        activity_stress = 2.5  # Higher stress if sedentary
    else:  # moderate
        activity_stress = 2.0  # Baseline
    
    # -----------------------------------------------------------------------------
    # Calculate combined stress score
    # -----------------------------------------------------------------------------
    avg_stress = (hr_stress + hrv_stress + sleep_stress + self_stress + activity_stress) / 5
    ...
    reason = (
        f"HR={heart_rate}, HRV={hrv}, Sleep={sleep_hours}h, "
        f"Activity={activity_level}, Combined={avg_stress:.2f}"
    )
