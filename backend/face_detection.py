...
# Minimum face area as a ratio of total image area to reject tiny false positives
# Prevents spurious detections on patterns, textures, or distant objects
MIN_FACE_AREA_RATIO = 0.002  # Face must be at least 0.2% of image
...
def detect_face_mediapipe(image: np.ndarray) -> Optional[Tuple[Tuple[int, int, int, int], int]]:
    ...
    Selects the face with largest area from all detections to handle
    multiple faces gracefully.
    ...
    try:
        ...
        with FaceDetection(model_selection=0, min_detection_confidence=0.5) as face_detection:
            ...
            # Select the detection with largest area (instead of just the first)
            best_detection = None
            best_area = 0
            h, w, _ = image.shape
            ...
            if best_detection is None:
                logger.info("MediaPipe: No valid detection after filtering")
                return None

            logger.info(f"MediaPipe: Detected face at {best_detection} (area={best_area})")
            return best_detection, len(results.detections)
...
def detect_face(image: np.ndarray) -> Optional[FaceDetectionResult]:
    ...
    Also applies minimum size filtering to reject false positives
    from tiny or irrelevant detections.
    ...
    - Size filtering prevents spurious detections
    ...
    h, w, _ = image.shape
    image_area = h * w
    min_face_area = image_area * MIN_FACE_AREA_RATIO
    ...
    #     else:
    #         logger.info(f"RetinaFace: Face too small (area={face_area}, min={min_face_area}), rejecting")
    ...
    if result is not None:
        ...
        if face_area >= min_face_area:
            ...
            logger.info(f"MediaPipe: Face too small (area={face_area}, min={min_face_area}), rejecting")
    ...
    if result is not None:
        ...
        if face_area >= min_face_area:
            ...
            logger.info(f"Haar: Face too small (area={face_area}, min={min_face_area}), rejecting")

    # All methods failed or all detections were too small
...
def detect_and_crop_face(pil_image: Image.Image) -> Tuple[Optional[Image.Image], Optional[FaceDetectionResult]]:
    ...
    try:
        ...
        # Convert back to PIL and detach it from the numpy view.
        cropped_image = Image.fromarray(cropped_array).copy()

        return cropped_image, detection
    finally:
        del image_array, cropped_array
        gc.collect()
...
class NoFaceDetectedError(Exception):
    ...
    """
    def __init__(self, message: str = "No face detected. Please align your face with the camera."):
